...
const IAM = () => {
  ...
  const audioTeclado = "/sounds/teclado.mp3";
  ...
  const handleUserInteraction = () => {
    if (!userHasInteracted) {
      ...
      // Opcional: reproducir el audio inmediatamente después del primer clic
      if (audioRef.current) {
        audioRef.current.play().catch(error => {
          console.log('Error al reproducir audio en primera interacción:', error);
        });
      }
    }
  };
  ...
  const typeWriter = () => {
    ...
    // Reproducir sonido solo en la primera carga y si el usuario ya interactuó
  ...
  useEffect(() => {
    // Agregar event listeners para detectar la primera interacción del usuario
    ...
    // Limpiar event listeners e intervalo al desmontar el componente
    return () => {
      events.forEach(event => {
  ...
  return (
    <section className="w-full justify-items-start py-16 px-4 mt-12">
      ...
      {/* Audio element para el sonido de teclado */}
      <audio ref={audioRef} preload="auto">
        <source src={audioTeclado} type="audio/mpeg" />
        <source src={audioTeclado.replace('.mp3', '.ogg')} type="audio/ogg" />
